...
class _CheckoutBodyState extends State<CheckoutBody> {
  ...
  String selectedTimeZone = 'WIB'; // Default Zone WIB
  ...
  // Convert time based on the selected time zone
  String convertTimeToZone(DateTime dateTime, String zone) {
    final utcTime = DateTime.now().toUtc();

    switch (zone) {
      case "WIB":
        return DateFormat('HH:mm').format(utcTime.add(Duration(hours: 7)));
      case "WITA":
        return DateFormat('HH:mm').format(utcTime.add(Duration(hours: 8)));
      case "WIT":
        return DateFormat('HH:mm').format(utcTime.add(Duration(hours: 9)));
      case "London":
        return DateFormat('HH:mm').format(utcTime); // UTC == London
      default:
        return DateFormat('HH:mm').format(utcTime); // fallback
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Ensure selectedItems is not empty
    if (selectedItems.isEmpty) {
      return Center(child: Text("Tidak ada produk yang dipilih"));
    }

    // Group products by store address
    ...
    return Padding(
      ...
      child: ListView(
        children: [
          ...
          // Display converted time based on selected zone
          Text(
            'Waktu di zona waktu $selectedTimeZone: ${convertTimeToZone(DateTime.now(), selectedTimeZone)}',
          ...
          Card(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  const Text('Alamat Toko', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  ...
                  ...groupedByStore.keys.map(
                    (storeAddress) => Padding(
                      ...
                      child: Row(
                        children: [
                          Icon(Icons.store, color: lightColorScheme.primary, size: 20),
                          const SizedBox(width: 10),
                          Expanded(
                            child: Text(storeAddress, style: TextStyle(fontSize: 16, color: Colors.grey.shade700)),
                          ),
                        ],
                      ),
                    ),
          ...
          Card(
            ...
            child: Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Opsi Ambil Pesanan Sendiri', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  Column(
                    children: [
                  ...
                  if (pickupOption == 'Terjadwal') ...[
                    ...
                    const Text('Pilih Tanggal', style: TextStyle(fontWeight: FontWeight.w600)),
                    ...
                    const SizedBox(height: 16),
                    const Text('Pilih Jam', style: TextStyle(fontWeight: FontWeight.w600)),
                    const SizedBox(height: 8),
                    Wrap(
                      spacing: 12,
                      runSpacing: 12,
                      children: List.generate(generateTimeSlots().length - 1, (index) {
                        final start = generateTimeSlots()[index];
                        final end = generateTimeSlots()[index + 1];
                        final label = '${start.format(context)} - ${end.format(context)}';
                      ...
                      return Padding(
                        ...
                        child: Row(
                          children: [
                            ...
                            Text(
                              'x$quantity',
                              style: TextStyle(fontWeight: FontWeight.w500, fontSize: 15, color: lightColorScheme.primary),
                            ),
                          ],
                        ),
                      );
